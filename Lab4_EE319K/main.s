...
;Enable Port F
	  ...
	  LDR R1, [R0]
	  ORR R1, #0x02		   ; Output on PF1
	  STR R1, [R0]
	  LDR R0, =GPIO_PORTF_DEN_R
	  LDR R1, [R0]
	  ORR R1, #0x02		   ; enable PF1
	  STR R1, [R0]
...
Debug_Init 
	...
	LDR R0, =DataBuffer
	MOV R2, #0
Dloop				;Set all entries of DataBuffer to 0xFF
	MOV R4, #0xFF
	STRB R4, [R0,R2]
	ADD R2, #1
	CMP R2, #99
	...
	LDR R0, =TimeBuffer
	MOV R2, #0
Tloop				;Set all entries of TimeBuffer to 0xFFFFFFFF
	MOV R4, #0xFFFFFFFF
	STR R4, [R0,R2]
	ADD R2, #4
	CMP R2, #396
...
Debug_Capture 
	...
	AND R3, #0x00FFFFFF
	STR R3, [R1]
	ADD R1, #4					;Increment Pointer by 16 (32 bit array)
	STR R1, [R0]
	
	LDR R0, =Data_Pt
	LDR R1, [R0]				;R1=Data pointer
	LDR R2, =GPIO_PORTE_DATA_R
	LDR R2, [R2]				;R2=PORT E Data
	AND R2, #0x0F				;Clear other bits
	LDR R3, =GPIO_PORTA_DATA_R
	LDR R3, [R3]
	AND R3, #0x10
	ORR R2, R3
	STRB R2, [R1]
	ADD R1, #1					;Increment pointer by 4 (8 bit array)
	STR R1, [R0]	
